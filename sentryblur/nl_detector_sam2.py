"""Legacy SAM 2 + Grounding DINO backend for the `prompt` feature.

Kept alongside the SAM 3.1 path as a fallback for users who can't access
the gated SAM 3 weights on Hugging Face. Selectable via
`sentryblur prompt --backend sam2`. DINO finds the target boxes from a
text prompt on frame 0; SAM 2's video predictor then propagates those
targets across every frame.

Heavy dependencies (torch, transformers, sam2) are imported lazily in
__init__ so users without the [prompt] extra + sam2 git install get a
clean ImportError at construction time, not at module import.
"""
...
class NLDetectorSam2:
    """Promptable detector via Grounding DINO + SAM 2. Operates on a frame
    directory (SAM 2's video predictor requirement) instead of single-frame
    ...
    def process_video(self, frame_dir: Path) -> list[np.ndarray]:
        ...
        detections = self.detect_boxes(first)
        if len(detections) == 0:
